...
public class Warrior extends BasicPiece {
    public Warrior(Color c) {
        ...
        if (color == WHITE) {
            move = new StepMove(FORWARD).addConstraint(formMoveConstraint(new StartLevelConstraint(2)));
            capture = new Or(
            ...
            move = new StepMove(BACKWARD).addConstraint(formMoveConstraint(new StartLevelConstraint(2)));
            capture = new Or(
